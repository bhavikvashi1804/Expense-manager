...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<Widget> _buildLandscapeContent(MediaQueryData mqData,AppBar appBar1,Widget txListWidget) {
    return [Row(
  ...
  List<Widget> _buildPotraitContent(
    MediaQueryData mqData, AppBar appBar, Widget txListWidget) {
    return [
      Container(
        height: (mqData.size.height -
    ...
    final pageBody = SafeArea(
      child: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            if (isLandscape) 
              ..._buildLandscapeContent(mediaQuery,appBar1,txListWidget),

            if (!isLandscape)
              ..._buildPotraitContent(mediaQuery, appBar1, txListWidget),
              //use spread operator else it gives error that widget child must be only one widget
